...
# Using the pre-trained dataset for  "Vaginal"
...
#save(IBD_simulation_1 , file = "IBD_simulation_1_sparseD.RData")

Sigma_full <- IBD_simulation_1$template$EM_fit$fit$Sigma
pi0_full <- IBD_simulation_1$template$EM_fit$fit$pi0
mu_full <- IBD_simulation_1$template$EM_fit$fit$m
sigma_full <- IBD_simulation_1$template$EM_fit$fit$sigma
...
# Run the simulation
sim_data <- simulate_multinomial_data(n = 500, pi = pi, mu = mu, sigma = sigma, R = R)

# Save the output with a unique file name using the task_id
save(sim_data,file=paste0("IBD_sim_data_", task_id, ".RData"))
