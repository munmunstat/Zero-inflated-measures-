############################################
# SLURM JOB SCRIPT: Spearman's rho Correlation for IBD sample n=500
############################################
library(VC2copula)
library(copula)
library(MASS)
...
sim_data_file <- paste0("IBD_sim_data_", task_id, ".RData")
...
# ---------------------------------------------
fit_data_file <- paste0("fit_IBD_data_", task_id, ".RData")
...
load("IBD_truspearman_matrix_result.RData")
# -----------------------------------------------
# Compare metrics
# -----------------------------------------------
...
lower_tri_trurho <- IBD_truspearman_matrix_result[lower.tri(IBD_truspearman_matrix_result)]
...
cat("Task", task_id, "MSE (rho star):", mse_rhostar, "\n")
cat("Task", task_id, "MAD (rho star):", mad_rhostar, "\n")

save.image(file = paste0("SI_simulation_task_", task_id, ".RData"))
