...
# Using the pre-trained dataset for "IBD"
IBD_simulation_1 <- SparseDOSSA2(template = "IBD",  # choose from "Stool", "Vaginal" or "IBD"
...
Sigma_full <- IBD_simulation_1$template$EM_fit$fit$Sigma
pi0_full <- IBD_simulation_1$template$EM_fit$fit$pi0
mu_full<-IBD_simulation_1$template$EM_fit$fit$mu
sigma_full<-IBD_simulation_1$template$EM_fit$fit$sigma
length(sigma_full)
# Adjust this vector as needed to select other features
selected_indices <- 51:100
...
IBD_truetau_matrix_result <- tauMatrix(mu, sigma, pi, R, tau)
print(IBD_truetau_matrix_result)
save(IBD_truetau_matrix_result, file = "IBD_truetau_matrix_result.RData")


#IBD_truetau_star<-calculate_tau_star(X=ziln_data[, 1],Y=ziln_data[, 2])
#print(IBD_truetau_star)
#save(IBD_truetau_star, file = "IBD_truetau_star.RData")

IBD_truetaustar_matrix_result<-kendallTaustar(ziln_data)
print(IBD_truetaustar_matrix_result)
save(IBD_truetaustar_matrix_result, file = "IBD_truetaustar_matrix_result.RData")
